...
// original source from: https://github.com/akauppi/scalajs-snapsvg, created by Asko Kauppi
...
trait SnapElement extends js.Object {
  ...
  //
  // modification: Slawek Kacprzykowski
  def attr( map: js.Dictionary[Any] ): this.type = ???   // returns "the current element"
  def attr( key: String ): js.Any = ???               // get an attribute (would that always be 'String' or can it be 'Number' / 'Boolean'?

//  def getBBox: SnapBBox = ???
  // modification: Slawek Kacrprzykowski
  def getBBox: js.Dictionary[Any] = ???
  // SNAPSVG DOCUMENTATION AMBIGUITY
  //
  ...
  //
  // modification: Slawek Kacprzykowski
  def animate( attr: js.Dictionary[Any], duration: Int /*ms*/ ): this.type = ???
...
trait SnapGroupElement extends SnapElement {
  // @author: slawek kacprzykowski
  def add(els: SnapElement*): SnapGroupElement = ???
}
...
trait SnapPaper extends js.Object {
  ...
  //
  def text( x: Double, y: Double, text: String ): SnapTextElement = ???
